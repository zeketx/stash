...
**Project:** ytdl - YouTube Downloader (Rust)

**Language:** Rust

**Purpose:** A YouTube downloader with both CLI and TUI interfaces. Built in Rust using yt-dlp as the backend.

**Key Dependencies:**
- yt-dlp (required): pip install yt-dlp
- FFmpeg (optional, for audio conversion): brew install ffmpeg

**Architecture:**
```
src/
├── core/          # Business logic (batch, history, playlist)
├── infra/         # External integrations (yt-dlp, clipboard, logger, notifications)
├── cli/           # CLI commands and parsing
├── tui/           # Terminal UI (screens, widgets, app state, event loop)
└── shared/        # Shared utilities (constants, error, utils)
```

**Key Features:**
- TUI terminal interface with visual feedback
- Batch downloads with concurrent processing
- Download history tracking (~/.ytdl/history.json)
- Clipboard support (auto-detect URLs)
- Audio extraction (MP3 conversion with FFmpeg)
- Playlist support (entire playlists or specific ranges)
- Desktop notifications
- Configurable (output directory, quality, etc.)

**Configuration:** ~/.ytdl/config.toml (supports environment variables)

**Build:** cargo build --release

**Testing:** cargo test

**Validation Steps for Rust:**

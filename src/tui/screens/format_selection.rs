...
pub fn render_format_selection(
    ...
    let info_text = vec![
        ...
        Line::from(vec![
            Span::styled("Uploader: ", Style::default().fg(theme.secondary).add_modifier(Modifier::BOLD)),
            Span::raw(&video_info.uploader),
        ]),
        Line::from(vec![
            Span::styled("Duration: ", Style::default().fg(theme.secondary).add_modifier(Modifier::BOLD)),
    ...
    let items: Vec<ListItem> = formats
        ...
        .map(|(i, format)| {
            ...
            let line = Line::from(vec![
                ...
                Span::raw("  "),
                Span::styled(&format.file_size, Style::default().fg(theme.secondary)),
            ]);
            ListItem::new(line)
        })
        .collect();
